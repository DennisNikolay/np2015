...
public class ColumnWorkerImpl extends ColumnWorkerAbstractImpl {
	...
	public ColumnWorkerImpl(TDoubleArrayList initialVertexValues, int column,
			GraphInfo ginfo, GlobalObserver o, Exchanger<TDoubleArrayList> el,
	...
	public void run() {
		...
		while (!shouldTerminate() && totalIterCounter != Integer.MAX_VALUE) {
			...
			for (int j = 0; j < vertex.size(); j++) {
				...
				if (left.size() != 0) {
					int leftY = getEncodedRowCoordinate(left.get(0)); // TODO j
																		// statt
																		// 0?
					if (leftY == y) {
					...
					if (leftY < y) { // if node propagated to from acc was 0
										// earlier
						double[] toAdd = { getActualValue(left.get(0), leftY)
								+ leftY * 10 }; // add new node
				...
				if (right.size() != 0) {
					...
					if (rightY < y) { // if node propagated to from acc was 0
										// earlier
						double[] toAdd = { getActualValue(right.get(0), rightY)
								+ rightY * 10 }; // add new node
						vertex.add(toAdd, j - 1, 1);
						right.remove(0, 1);
					}
				}
				...
				if (j != 0 && propagateTop != 0) {
					...
					if (backY == y - 1) { // Check if previous node in arraylist is wanted vertex
						...
						double[] toAdd = { propagateTop + (y - 1) * 10 }; // else
																			// add
																			// new
																			// node
				...
				} else if (j == 0 && propagateTop != 0) { // if added node is at
															// beginning of list
					double[] toAdd = { propagateTop + (y - 1) * 10 }; // add new
																		// node
				...
				if (j == vertex.size() - 1 && propagateBottom != 0) {
					...
					if (forwardY != y + 1) {
						...
						vertex.set(j + 1, getActualValue(nodeForward, forwardY)
								+ propagateBottom + forwardY * 10); // if so set
																	// correct
																	// value
					}
				}
...
//					System.out.println(j + ": " + vertexValue);
			...
			if (left.size() > 0) { // All exchanged Acc values should done by
									// now, if not, then they are at the end of
									// vertex list and 0 till now
