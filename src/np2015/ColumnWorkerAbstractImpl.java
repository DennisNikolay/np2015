...
/**
 * Extend this to implement local ColumnWorker No Implementation of actual run
 * method in this class No implementation of exchangeLeft/exchangeRight - YET -
...
public abstract class ColumnWorkerAbstractImpl extends Observable implements
	...
	/**
	 * Edges encoded by position, 4 per vertex, 0 mod 4 is left, 1 mod 4 is
	 * right, 2 mod 4 is top, 3 mod 4 is bottom.
	 */
	 ...
	 *            - make sure keys inserted in order
	 * @param column
	 */
	public ColumnWorkerAbstractImpl(
			HashMap<Integer, Double> initialVertexValues, int column,
			GraphInfo ginfo, GlobalObserver globalChecker,
	...
	public ColumnWorkerAbstractImpl(TDoubleArrayList initialVertexValues,
			int column, GraphInfo ginfo, GlobalObserver globalChecker,
			Exchanger<TDoubleArrayList> el, Exchanger<TDoubleArrayList> er) {
		this(column, ginfo, globalChecker, el, er);
		vertexValues = initialVertexValues;
	}

	public ColumnWorkerAbstractImpl(int column, GraphInfo ginfo,
	...
	public TDoubleArrayList exchangeLeftAccValues() {
		if (getColumnIndex() != 0) {
			...
			if (getColumnIndex() != 0 && leftExchanger == null
					&& !leftAccValues.isEmpty()) {
				leftExchanger = new Exchanger<TDoubleArrayList>();
				Runnable r = new ColumnWorkerImpl(leftAccValues,
						columnIndex - 1, NPOsmose.ginfo, NPOsmose.o, null,
						leftExchanger);
				new Thread(r).start();
	...
	public double getRateForTarget(int x, int y, Neighbor where) {
		if (x != columnIndex) {
			throw new UnsupportedOperationException(
					"Trying to getRate for Column, that is not the Column worked by this Thread");
		} else {
			int n = 0;
			switch (where) {
			case Left:
